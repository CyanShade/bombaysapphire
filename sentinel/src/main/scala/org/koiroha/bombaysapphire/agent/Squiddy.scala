...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
 * マニュアル/オート哨戒可能な WebView を表示する。
 *
 * @author Takami Torao
 */
private class Squiddy(config:Config, proxy:EmbeddedProxy) {
	import org.koiroha.bombaysapphire.agent.Squiddy._
	...
	private[this] class Scenario(conf:Config, engine:WebEngine, browser:Closeable) {
		...
		private[this] val waypoints = conf.newWayPoints{ _ => false }
...
object Squiddy {
	private[Squiddy] val logger = LoggerFactory.getLogger(classOf[Squiddy])
